...
//
void MD5Digest::Process(const md5_byte_t *data /*[64]*/) {
	...
	if (GetHostEndianness() == LittleEndian) {
		/*
...
void MD5Digest::Append(const md5_byte_t *data, size_t size) {
	const md5_byte_t *p = data;
	int left = int(size),
		offset = (count[0] >> 3) & 63;
	md5_word_t nbits = (md5_word_t)(size << 3);

	if (size <= 0)
		return;

	// Update the message length.
	count[1] += md5_word_t(size >> 29);
	count[0] += nbits;
	if (count[0] < nbits)
		count[1]++;
	...
	if (offset) {
		const auto copy = int(offset + size > 64 ? 64 - offset : size);
...
void MD5Digest::Finish(md5_byte_t digest[16]) {
...
void MD5DigestToString(const md5_byte_t digest[16], char *s) {
